...
class RLAgent:
    ...
    def next_move(self, board):
        ...
        candidate_moves = []  # store all posibible moves
        for y in range(3):
            for x in range(3):
                if board[y, x] == 0:  # AI can take here
                    ...
                    candidate_moves.append(
                        [hashkey, self.state_values[hashkey], (x, y)])
    ...
    def update_state_values(self, last_state_value):
        lr = 0.2
        N = len(self.history_states)
        for i in range(N-1, -1, -1):  # update value function from the last state
            cur_state = self.history_states[i]
            if i == N-1:
                self.state_values[cur_state] = last_state_value
            else:
                nex_state = self.history_states[i+1]
                # Temporal-difference learning
                # V(s) <- V(s) + lr * [V(s') - V(s)]
                self.state_values[cur_state] += lr * \
                    (self.state_values[nex_state] -
